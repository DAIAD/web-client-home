...
var connectActionsToQueryBackend = function connectActionsToQueryBackend(QueryBackend) {
  ...
  */
  ...
  var queryDeviceSessions = function queryDeviceSessions(options) {
    return function (dispatch, getState) {
      return dispatch(QueryBackend.queryDeviceSessions(options)).then(function (sessions) {
        return sessions.map(function (session) {
          return _extends({}, session, {
  ...
  var fetchLastDeviceSession = function fetchLastDeviceSession(options) {
    return function (dispatch, getState) {
      return dispatch(QueryBackend.queryDeviceSessions(_extends({}, options, { length: 1 }))).then(function (response) {
        ...
        return dispatch(QueryBackend.fetchDeviceSession({ id: id, deviceKey: device })).then(function (session) {
          return _extends({}, session, {
            active: [device, id],
            showerId: id,
            device: device,
  ...
  var queryUserComparisonsByTime = function queryUserComparisonsByTime(options) {
    return function (dispatch, getState) {
      ...
      return Promise.all(Array.from({ length: iters }, function (x, i) {
        ...
        return dispatch(QueryBackend.queryUserComparisons({ userKey: userKey, month: month, year: year }));
      }));
    };
  };
  ...
  var fetchUserComparison = function fetchUserComparison(options) {
    return function (dispatch, getState) {
      ...
      return dispatch(QueryBackend.queryUserComparisonsByTime({ userKey: userKey, time: time })).then(function (comparisonsArr) {
  ...
  var fetchWidgetData = function fetchWidgetData(options) {
    return function (dispatch, getState) {
      ...
      var time = options.time ? options.time : timeUtils.getTimeByPeriod(period, periodIndex);

      if (deviceType === 'METER') {
        var _ret = function () {
          var prevTime = timeUtils.getPreviousPeriodSoFar(period, time.startDate);
          return {
            v: dispatch(queryMeterHistory({
              userKey: userKey,
              time: time
            })).then(function (data) {
              return { data: data };
              ...
              if (type === 'total' && prevTime) {
                ...
                return _extends({}, res, { brackets: brackets });
              } else if (type === 'breakdown') {
                return _extends({}, res, { breakdown: breakdown });
              }
        ...
        return dispatch(QueryBackend.queryDeviceSessions({
          ...
          return period !== 'all' ? dispatch(QueryBackend.queryDeviceSessions({
            length: genUtils.showerFilterToLength(period),
            deviceKey: deviceKey,
            userKey: userKey,
            index: -1
