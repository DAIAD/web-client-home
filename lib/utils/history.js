...
var getStatsMeterData = function getStatsMeterData(props) {
  ...
  var comparisonsData = props.comparisons.map(function (comparison) {
    ...
    return {
      name: getComparisonTitle(props.activeDeviceType, comparison.id, props.time.startDate, props.timeFilter, props.favoriteCommonName, props.members, props.intl),
...
var getStatsAmphiroData = function getStatsAmphiroData(props) {
  ...
  var comparisonsData = props.comparisons.map(function (comparison) {
    return comparison.sessions.map(function (dev) {
      return {
        name: getComparisonTitle(props.activeDeviceType, comparison.id, props.time.startDate, props.timeFilter, props.favoriteCommonName, props.members, props.intl) + ' (' + dev.name + ')',
...
var getWaterIQData = function getWaterIQData(props) {
  ...
  var chartData = [{
    ...
    return {
      name: c.title || getComparisonTitle(props.activeDeviceType, c.id, props.time.startDate, props.timeFilter, props.favoriteCommonName, props.members, props.intl),
