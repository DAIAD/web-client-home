...
var getTimeByPeriod = function getTimeByPeriod(period, index) {
  if (period === 'year') return getYear(moment().add(index, period).valueOf());else if (period === 'month') return getMonth(moment().add(-3, period).valueOf());else if (period === 'week') return getWeek(moment().add(index, period).valueOf());else if (period === 'day') return getDay(moment().add(index, period).valueOf());
  return {};
};
