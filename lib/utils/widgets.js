...
var tip = function tip(widget, intl) {
  var tips = widget.tips,
      _widget$tipIndex = widget.tipIndex,
      tipIndex = _widget$tipIndex === undefined ? 0 : _widget$tipIndex;

  var currTip = Array.isArray(tips) && tips.length > tipIndex && tips[tipIndex];
  ...
  var text = description.substring(0, String(description).indexOf('.', 120));
  return _extends({}, widget, {
    highlight: {
...
var amphiroOrMeterTotal = function amphiroOrMeterTotal(widget, intl) {
  ...
  console.log('meter curr', time, data, reduced);
  console.log('meter prev', widget.prevTime, previous, previousReduced);
...
var meterForecast = function meterForecast(widget, intl) {
  ...
  var forecastChartData = Array.isArray(forecastData) ? [{
    ...
    data: getChartMeterData(forecastData, xCategories, time, metric),
    lineType: 'dashed',
    color: '#2d3480',
    fill: 0.1,
