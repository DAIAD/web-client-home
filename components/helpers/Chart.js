...
const Chart = React.createClass({
  ...
  //TODO: check data needs reimport!
  ...
  _getPieOptions: function (options) {
    return {
      title: {
        ...
        padding: this.props.title && this.props.title.length ?
          [-2, 0, 50, 30] : 5,
        x: 'center',
        y: 'center',
        textStyle: {
  ...
  _getBaseOptions: function (options) {
    return {
      title: {
        text: options.title,
        padding: options.title && options.title.length ?
          [-2, 0, 50, 30] : 5,
        textStyle: {
          //fontFamily: "OpenSansCondensed",
          color: '#808285'
      ...
      tooltip: {
        formatter: params => options.xAxis === 'time' ? 
          `${moment(params.value[0]).format('MMMM Do YYYY, h:mm:ss a')}:` +
            ` ${params.value[1]} ${options.mu}` 
            : `${params.name}: ${params.value} ${options.mu}`,
        trigger: 'item',
